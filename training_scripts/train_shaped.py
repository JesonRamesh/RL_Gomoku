...
def train_dqn(
    ...
    minimax_opp = MinimaxAgent(
        player_id=-1, board_size=board_size, time_limit=0.05, skill_level=0.5
    )
    ...
    difficulty_levels = [
        ...
        (
            "Strat-0.2",
            lambda: setattr(strategic_opp, "skill_level", 0.2) or strategic_opp,
            1,
        ),
        (
            "Strat-0.4",
            lambda: setattr(strategic_opp, "skill_level", 0.4) or strategic_opp,
            2,
        ),
        (
            "Strat-0.5",
            lambda: setattr(strategic_opp, "skill_level", 0.5) or strategic_opp,
            3,
        ),
        (
            "Strat-0.6",
            lambda: setattr(strategic_opp, "skill_level", 0.6) or strategic_opp,
            4,
        ),
        (
            "Strat-0.7",
            lambda: setattr(strategic_opp, "skill_level", 0.7) or strategic_opp,
            5,
        ),
        (
            "Strat-0.8",
            lambda: setattr(strategic_opp, "skill_level", 0.8) or strategic_opp,
            6,
        ),
    ...
    print("=" * 70)
    print("DQN TRAINING - SHAPED REWARDS - Shaped Rewards + Adaptive Curriculum")
    print("=" * 70)
    ...
    for episode in range(total_episodes):
        ...
        if len(recent_wins) >= 50 and games_at_level >= MIN_GAMES:
            ...
            if (
                win_rate >= PROMOTE_THRESHOLD
                and current_level < len(difficulty_levels) - 1
            ):
                current_level += 1
                games_at_level = 0
                recent_wins.clear()
                ...
                if current_level > max_level:
                    ...
                    _save_policy_weights(
                        agent, os.path.join(save_dir, f"level_{current_level}.pt")
                    )
        ...
        if (episode + 1) % 500 == 0:
            ...
            print(
                f"Ep {episode + 1:>6}/{total_episodes} | "
        ...
        if (episode + 1) % 2000 == 0:
            ...
            wr_strat = evaluate_agent(
                agent, StrategicAgent(-1, 0.5, board_size), board_size, 25
            )
            wr_mm = evaluate_agent(
                agent,
                MinimaxAgent(
                    -1, board_size=board_size, time_limit=0.05, skill_level=0.5
                ),
                board_size,
                20,
            )
            ...
            print(
                f"vs Random: {wr_rand:.0f}% | vs Strat-0.5: {wr_strat:.0f}% | vs MM-0.5: {wr_mm:.0f}%"
            )
    ...
    final_strat03 = evaluate_agent(
        agent, StrategicAgent(-1, 0.3, board_size), board_size, 50
    )
    final_strat05 = evaluate_agent(
        agent, StrategicAgent(-1, 0.5, board_size), board_size, 50
    )
    final_strat07 = evaluate_agent(
        agent, StrategicAgent(-1, 0.7, board_size), board_size, 50
    )
    final_mm03 = evaluate_agent(
        agent,
        MinimaxAgent(-1, board_size=board_size, time_limit=0.1, skill_level=0.3),
        board_size,
        30,
    )
    final_mm05 = evaluate_agent(
        agent,
        MinimaxAgent(-1, board_size=board_size, time_limit=0.1, skill_level=0.5),
        board_size,
        30,
    )
    final_mm07 = evaluate_agent(
        agent,
        MinimaxAgent(-1, board_size=board_size, time_limit=0.1, skill_level=0.7),
        board_size,
        30,
    )
    ...
    plt.plot(
        eval_points, eval_minimax, "mp-", linewidth=2, markersize=4, label="vs MM-0.5"
    )
    plt.xlabel("Episode")
    plt.ylabel("Win Rate (%)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--episodes", type=int, default=80000, help="Total episodes (default 80000)"
    )
    parser.add_argument(
        "--load-from",
        type=str,
    ...
    parser.add_argument(
        "--save-dir", type=str, default="models_shaped", help="Directory to save models"
    )
